...
min_ssim_score_list = [0.56, 0.4, 0.5, 0.5, 0.5, 0.56, 0.54]
max_allowed=[0.7, 0.44, 0.64, 0.64, 0.66, 0.6, 0.67]



def sort_descending_inplace(input_list):
    input_list.sort(reverse=True)

def verify_100(test_img):
...
def testFeature_1_2_7( test_img, blur_test_img,gray_test_image ):
    ...
    for j in range(NUM_OF_FEATURES):
        ...
        for i in range(NUMBER_OF_TEMPLATES):
            ...
            template_path = r'Dataset\100_Features Dataset\Feature ' + str(j+1) + '\\' +  str(i+1)+ '.jpg' 
    ...
    for i in range(len(score_set_list)):
        size=0
        sumi=0
        sort_descending_inplace(score_set_list[i])
